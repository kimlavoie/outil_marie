...
function generateExcelWorkbook(xlsxInstance?: any) {
...
function runExportToExcel(_getExcelColName: (colIdx: number) => string, xlsxInstance?: any) {
  try {
    const lib = xlsxInstance || (globalThis as any).XLSX;
    const wb = generateExcelWorkbook(lib);
