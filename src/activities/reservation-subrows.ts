...
// Builds the "Compte à facturer" options for a service's billing_accounts list (the named
// budget accounts configured in the Équipements settings, e.g. one for internal clients, one for
// external ones). A service with no configured accounts falls back to a single "Aucun" option.
function buildServiceGlAccountOptionsHtml(service: { billing_accounts?: { label: string; gl_account_code: string }[] } | undefined, selectedCode = ""): string {
  const billingAccounts = (service && service.billing_accounts) || [];
  if (billingAccounts.length === 0) {
    return '<option value="">Aucun</option>';
  }
  return billingAccounts
    .map(a => {
      const label = a.label ? `${a.label} (${a.gl_account_code})` : a.gl_account_code;
      return `<option value="${a.gl_account_code}" ${a.gl_account_code === selectedCode ? "selected" : ""}>${escapeHtml(label)}</option>`;
    })
    .join("");
}

export function addServiceRow(container: HTMLElement, serviceId = "", count = 1, hours = 0, glAccountCode = "", autoGenerated = false) {
  ...
  container.insertAdjacentHTML(
    ...
    <div id="${rowId}" class="distribution-row" data-auto-generated="${autoGenerated ? "1" : ""}" style="grid-template-columns: 1.3fr 0.6fr 0.6fr 1fr 1fr auto;">
      ...
      <select id="${rowId}-gl-account" class="select-input service-gl-account-select" title="Compte à facturer" style="padding: 8px 12px; font-size: 0.85rem;">
        ${buildServiceGlAccountOptionsHtml(initialService, glAccountCode)}
      </select>
  ...
  row.querySelector<HTMLSelectElement>(".service-select")!.addEventListener("change", e => {
    ...
    row.querySelector<HTMLSelectElement>(".service-gl-account-select")!.innerHTML = buildServiceGlAccountOptionsHtml(service);
  });
  row.querySelectorAll<HTMLInputElement>("select, input")!.forEach(el => {
    el.addEventListener("input", updateSubmissionFinancialSummary);
    el.addEventListener("change", updateSubmissionFinancialSummary);
...
export function collectServicesFromForm(card: HTMLElement) {
  return Array.from(card.querySelectorAll<HTMLInputElement>(".room-services-list .distribution-row")!)
    .map(row => ({
      ...
      gl_account_code: row.querySelector<HTMLSelectElement>(".service-gl-account-select")!.value,
      auto_generated: row.dataset.autoGenerated === "1"
    }))
