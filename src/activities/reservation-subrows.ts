...
// Builds the "Tarif" options for a service's tarifs list (each a named price point configured in
// the Équipements settings, e.g. one for internal clients, one for external ones, carrying its
// own budget account). A service with no configured tarifs falls back to a single "Aucun" option.
function buildServiceTarifOptionsHtml(service: { tarifs?: { id: string; label: string; gl_account_code: string }[] } | undefined, selectedTarifId = ""): string {
...
export function addServiceRow(container: HTMLElement, serviceId = "", count = 1, hours = 0, tarifId = "", autoGenerated = false) {
  ...
  container.insertAdjacentHTML(
    ...
    <div id="${rowId}" class="distribution-row" data-auto-generated="${autoGenerated ? "1" : ""}" style="grid-template-columns: 1.3fr 0.6fr 0.6fr 1fr 1fr auto;">
      ...
      <select id="${rowId}-tarif" class="select-input service-tarif-select" title="Tarif à facturer" style="padding: 8px 12px; font-size: 0.85rem;">
        ${buildServiceTarifOptionsHtml(initialService, tarifId)}
      </select>
  ...
  row.querySelector<HTMLSelectElement>(".service-select")!.addEventListener("change", e => {
    ...
    row.querySelector<HTMLSelectElement>(".service-tarif-select")!.innerHTML = buildServiceTarifOptionsHtml(service);
  });
  row.querySelectorAll<HTMLInputElement>("select, input")!.forEach(el => {
    el.addEventListener("input", updateSubmissionFinancialSummary);
    el.addEventListener("change", updateSubmissionFinancialSummary);
