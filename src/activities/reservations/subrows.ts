...
export function addStaffRow(
  ...
  container.insertAdjacentHTML(
    ...
    <div id="${wrapperId}" class="distribution-row-wrapper" data-auto-generated="${autoGenerated ? "1" : ""}" style="margin-bottom: 8px; width: 100%;">
      <div id="${rowId}" class="distribution-row" style="grid-template-columns: 1.2fr 65px 95px 95px 75px 75px 1.2fr 1fr 50px 100px 38px; align-items: center;">
...
export function addServiceRow(
  ...
  container.insertAdjacentHTML(
    ...
    `
    <div id="${wrapperId}" class="distribution-row-wrapper" data-auto-generated="${autoGenerated ? "1" : ""}" style="margin-bottom: 8px; width: 100%;">
      <div id="${rowId}" class="distribution-row" style="grid-template-columns: 1.3fr 0.6fr 1fr 1fr 50px 38px; align-items: center;">
...
export function collectStaffFromForm(card: HTMLElement) {
  ...
  const dropped = rows.filter(
    s =>
      !s.salary_id &&
      (s.count > 0 || s.hours > 0 || s.overtime_hours > 0 || s.tarif_id === "__custom__" || s.custom_rate > 0 || s.custom_overtime_rate > 0)
  );
  if (dropped.length > 0) {
...
export function collectServicesFromForm(card: HTMLElement) {
  const rows = Array.from(card.querySelectorAll<HTMLInputElement>(".room-services-list .distribution-row")!).map(row => {
    ...
    return {
      ...
      custom_gl_account_code: wrapper
