...
const SERVICE_HOURLY = {
  ...
  name: "Projecteur",
  type: "hourly",
  tarifs: [{ id: "tarif-hourly", label: "", gl_account_code: "GL-SVC", rate_versions: [{ id: "rv-hourly", effective_date: "", rate: 20 }] }]
};
...
test("addServiceRow rates an hourly service by hours x count and shows the hours input", () => {
  const container = freshContainer();
  addServiceRow(container, "service-hourly", 3, 2, "tarif-hourly");
...
test("collectServicesFromForm reads back service rows and warns on incomplete ones", () => {
  ...
  addServiceRow(servicesList, "service-hourly", 1, 3, "tarif-hourly");
  ...
  assert.equal(services[0].service_id, "service-hourly");
  assert.equal(getIncompleteRowWarnings().length, 1);
});
